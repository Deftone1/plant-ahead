...
function SavedPlants(props) {
  ...
  const plantList = plants
    .map((plant)=> (
      <p className="card-text">{plant.name} <br></br><a href="#" className="btn btn-outline-success btn-sm" id={plant._id}>Add to Project</a><a href="#" className="btn btn-outline-danger btn-sm"id={plant._id} onClick={()=>props.removeplant(plant._id)}>Remove Plant</a></p>
    ))
